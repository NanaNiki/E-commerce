...
const footertags = [
  ...
  { name: "Return Policy", url: "https://i.giphy.com/media/a93jwI0wkWTQs/giphy.webp" },
  ...
  { name: "Privacy Policy", url: "https://64.media.tumblr.com/cc3541eb31ca0bdaa6fce7cc77221e93/tumblr_ppvoy3dN7H1qisc9uo1_500.gif" },
  { name: null} 
];
      ...
      <div className="grid grid-cols-3 sm:w-3/4 w-11/12 ">        
          {footertags.map((tag, index) => {
            return (
              ...
              <span className="sm:text-sm text-xs text-stone-400">{tag.title}</span>
              <Link href={`${tag.url}`} passHref target="_blank">
              <span className={`lg:text-base sm:text-sm text-xs hover:text-stone-500 ${castoro.className} `}>{tag.name}</span>
              </Link>
              </div>
            )
          })}
