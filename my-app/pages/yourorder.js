...
export default function SeeYourOrder() {
  ...
  return (
    <div className={`mt-14 w-6/12 mx-auto ${castoro.className}`}>
      <h1 className="text-center pt-10 text-2xl font-bold text-stone-600">Your Order</h1>
      <h2 className="p-5 mx-auto text-2xl flex flex-row w-fit"> New Plants:</h2>
      <div className="flex flex-row justify-center">
        {orderItems.map((item) => (
          <div className="flex flex-col">
            <Image src={item.image} alt={item.name} width={60} height={70} className="mx-4 mb-2"/>
            <p className="text-center my-auto">{item.quantity} of</p>
            <p key={item.id} className="text-center my-auto">{item.name} </p>
          </div>
        ))}
      </div>

      <h2 className="pt-10 pb-5 text-2xl">Your Details:</h2>
      ...
      <button onClick={() => window.history.back()} className="pt-2 px-14 m-auto cursor-pointer bg-black w-60 h-10 text-white sm:text-base hover:bg-stone-700">Go Back</button>
      </div>
    </div>
  );
}
